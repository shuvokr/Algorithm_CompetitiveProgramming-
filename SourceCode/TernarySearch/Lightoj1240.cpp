...
template <class T> T cordinatlenth3D(T a, T b, T c, T d, T e, T f) { return sq(d - a) + sq(e - b) + sq(f - c); }
...
double x1, y11, z1, x2, y2, z2, x, y, z, a, b, c;
...
void input()
{
    ...
    while( T-- )
    {
        sf("%lf %lf %lf %lf %lf %lf %lf %lf %lf", &x1, &y11, &z1, &x2, &y2, &z2, &x, &y, &z);
        double high = 1.0, low = 0.0;
        a = x2 - x1; b = y2 - y11; c = z2 - z1;

        for(int i = 0; i < 100; i++)
        {
            double p1 = low + ((high - low) / 3);
            double p2 = low + (2.0 * (high - low) / 3);

            double ret1 = fun( p1 );
            double ret2 = fun( p2 );

            if(ret1 < ret2) high = p2;
            else low = p1;
        }
        pf("Case %d: %.8lf\n", ++kag, sqrt(fun((low + high) / 2.0)));
    }
}
double fun(double tim)
{
    ...
    return cordinatlenth3D(s1, s2, s3, x, y, z);
}
