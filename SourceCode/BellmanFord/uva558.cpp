...
int dis[ 3010 ], node, edge, u[ 3010 ], v[ 3010 ], cost[ 3010 ];
...
void Input()
{
    ...
    {
        sf("%d %d", &node, &edge);
        for(int i = 1; i <= node; i++) dis[ i ] = 1000000000;
        for(int i = 1; i <= edge; i++) sf("%d %d %d", &u[ i ], &v[ i ], &cost[ i ]);
        bool ans = false; dis[ 1 ] = 0;
        ...
        puts(ans ? "possible" : "not possible");
        //If Negetive cycle print possible else not possible
    }
}
